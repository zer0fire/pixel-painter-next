...
function PixelGrid({ onPickColor, currentColor, onPixelClick, socket }: Props) {
  ...
  const handleWrapperMouseup = (e: any) => {
  ...
  useEffect(() => {
    ...
    socket.on("initial-pixel-data", async (pixelData: any) => {
    ...
    socket.on("update-dot", ({ row, col, color }: any) => {
  ...
  const draw = (row: any, col: any, color: any) => {
    if (ctx.current) {
